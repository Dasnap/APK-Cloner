...
# # Get the last character of the package name
# LAST_CHAR=${OLD_PACKAGE_NAME: -1}

# # Increment the last character by one place in the alphabet
...
# echo "Replace the old package name with the new one in apktool.yml"
# sed -i "s/^renameManifestPackage:.*/renameManifestPackage: $NEW_PACKAGE_NAME/" $DECOMPILED_DIR/apktool.yml
...
# Check if a key with the same alias already exists
